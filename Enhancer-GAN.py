...
class WGAN_LangGP():
    def __init__(self, args):
        ...
        self.start_cutoff = args.start_cutoff
        self.add_cutoff = args.add_cutoff
        self.up_count = args.up_count

        self.mmd_cutoff = args.mmd_cutoff
    ...
    def log_param(self):
        ...
        log_dict["mmd_cutoff"] = self.mmd_cutoff
        log_dict["retrain"] = self.retrain
        log_dict["iteration"] = self.iteration
        log_dict["load_dir"] = self.load_dir
        log_dict["start_cutoff"] = self.start_cutoff
        log_dict["add_cutoff"] = self.add_cutoff
        log_dict["up_count"] = self.up_count
    ...
    def train_model(self):
        ...
        indexes = list(set(np.random.randint(0, len(self.data), 20100).tolist()))
        ...
        self.real_seqs_act = real_seqs_act
        self.val_seqs_act = val_seqs_act
            ...
            if epoch >= 5:
                ...
                if mmd_value < self.mmd_cutoff:
                    ...
                    if len(good_indices) > 0:
                        ...
                        # action 
                        if counter_cutoff % self.up_count == 0:
                            if self.start_cutoff < 2.5:
                                self.start_cutoff += self.add_cutoff
                            else:
                                act_mean = np.mean(preds_dev)
                                if self.start_cutoff < act_mean:
                                    self.start_cutoff = act_mean+(self.add_cutoff/4.0)
                    ...
                    f.write("epoch: {}; get number: {}; number_exchange: {}, current_cutoff: {:.5f}\n".format(epoch, len(pos_seqs), counter_cutoff, self.start_cutoff))
    ...
    def evaluate_model(self, num=10000, epoch=1):
        ...
        real_probability = Activity_Probability(self.real_seqs_act[0])
        val_probability = Activity_Probability(self.val_seqs_act[0])
    ...
    def sample(self, n_iters):
        ...
        for iter in range(n_iters):
            sequences = self.sample_generator(128).to("cpu").tolist()
            sequences = [decode_one_seq_2(seq, self.inv_charmap) for seq in sequences]
...
def main():
    ...
    parser.add_argument("--run_name",       default= "Enhancer-GAN", type=str)
    ...
    parser.add_argument("--start_cutoff",   default=-3.0, type=float)
    parser.add_argument("--add_cutoff",     default=0.05, type=float)
    parser.add_argument("--up_count",       default=5, type=float)
    parser.add_argument("--mmd_cutoff",     default=0.3, type=float)
